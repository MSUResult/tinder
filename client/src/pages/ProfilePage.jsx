...
const ProfilePage = () => {
  const { authUser, setAuthUser } = useAuthStore(); // Changed this to fetch setAuthUser for state update
  const { updateProfile } = useUserStore(); // Make sure you're calling updateProfile from the correct store
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call updateProfile from the store and update authUser accordingly
      await updateProfile({ name, bio, age, gender, genderPreference, image });
      setAuthUser({ name, bio, age, gender, genderPreference, image }); // Update auth state locally
    } catch (error) {
                  ...
                  {["Male", "Female", "Both"].map((option) => (
                    <label key={option} className="inline-flex items-center">
                      <input
                        ...
                        checked={genderPreference.includes(
                          option.toLowerCase()
                        )}
                        onChange={(e) =>
                          setGenderPreference((prev) =>
                            e.target.checked
                              ...
                              : prev.filter((p) => p !== option.toLowerCase())
                          )
                        }
                      />
                      <span className="ml-2">{option}</span>
